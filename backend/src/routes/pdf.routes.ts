...
router.get('/report-card/:id', authorize('ADMIN', 'FACULTY'), async (req, res, next) => {
  try {
    ...
    if (isSecondary) {
      ...
      let bannerBase64 = '';
      try {
        const p = 'c:\\Users\\chava\\OneDrive\\Documents\\Desktop\\ERP\\frontend\\public\\school_header_banner.png';
        if (fs.existsSync(p)) {
          bannerBase64 = `data:image/png;base64,${fs.readFileSync(p).toString('base64')}`;
        }
      } catch (err) {
      ...
      if (stud.photo) {
        try {
          const absolutePhotoPath = path.join('c:\\Users\\chava\\OneDrive\\Documents\\Desktop\\ERP\\backend\\public', stud.photo);
          if (fs.existsSync(absolutePhotoPath)) {
            studentPhotoBase64 = `data:image/jpeg;base64,${fs.readFileSync(absolutePhotoPath).toString('base64')}`;
          }
        } catch (err) {
